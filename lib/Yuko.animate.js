  ...
  Yuko.utility = (function () {
    ...
    function animate(ele, options) {
      ...
      var mainEntry = (function () {
        // Initial parameters
        var style, count = 0, index = 0, per = 0, origin = [], target = [], anim, keyframes = [], animType,
          supportProps = {
        ...
        for (var p in props) {
          ...
          console.log(parseFloat(props[p]) / 100 * document.documentElement.clientHeight);
        ...
        for (var i = 0; i < count; i++) {
          var tempKeyFrames = [], tempKeyFrame = 0, gap = target[i] - origin[i];
          // console.log('gap = ' + gap);
